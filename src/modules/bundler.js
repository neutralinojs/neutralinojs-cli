...
async function createAsarFile() {
  ...
  await fse.copy(`./${resourcesDir}`, `.tmp/${resourcesDir}`, {
    overwrite: true,
    filter:
      configObj.cli.resourcesExclude &&
  ...
  if (extensionsDir && fs.existsSync(extensionsDir)) {
    await fse.copy(
      ...
      `${buildDir}/${binaryName}/${extensionsDir}`,
      {
        overwrite: true,
        filter:
          configObj.cli.extensionsExclude &&
          (src => !new RegExp(configObj.cli.extensionsExclude).test(src)),
      },
    )
  }
  ...
  await fse.copy(
    ...
    `.tmp/${constants.files.configFile}`,
    { overwrite: true },
  )
  if (clientLibrary) {
...
module.exports.bundleApp = async (isRelease, copyStorage, isEmbedded) => {
  let configObj = config.get()
  let binaryName = configObj.cli.binaryName
  const buildDir = configObj.cli.distributionPath
    ? utils.trimPath(configObj.cli.distributionPath)
    : 'dist'
  const hostProjectConfig = configObj.cli
    ? configObj.cli.hostProject
    : undefined
  ...
  try {
    ...
    try {
      await Promise.all(
        Object.keys(constants.files.binaries.win32).map(async arch => {
      ...
      for (let platform in constants.files.binaries) {
        for (let arch in constants.files.binaries[platform]) {
          ...
          let destinationBinaryFile = hostproject.hasHostProject()
            ? `bin/${originalBinaryFile}`
            : originalBinaryFile.replace('neutralino', binaryName)
          bufferArr.push(fse.readFileSync(`${buildDir}/${binaryName}/${destinationBinaryFile}`))
          bufferArr.push(
            Buffer.from(`/${constants.files.resourceFile}`, 'utf-8'),
          )
          bufferArr.push(
            fse.readFileSync(`${buildDir}/${binaryName}/${constants.files.resourceFile}`),
          )
          let startpos = Buffer.alloc(8)
          startpos.writeBigInt64LE(BigInt(bufferArr[0].length))
          bufferArr.push(startpos)
    ...
    if (copyStorage) {
      ...
      try {
        ...
        utils.error(
          'Unable to copy storage data from the .storage directory. Please check if the directory exists',
        )
        process.exit(1)
      }
    }
    ...
    if (
      hostproject.hasHostProject() &&
      hostProjectConfig &&
      hostProjectConfig.buildPath
    ) {
